...
class ros_node(Node):
    def __init__(self):
        super(ros_node, self).__init__('feedback_node')
        ...
        self.theta_noise = np.random.normal(0.003, 0.003)
        ...
        self.control_command = self.create_subscription(Float32MultiArray, 'feedback_encoder', self.control_callback, 100)
        self.timer_integral = self.create_timer(1/130, self.integral)
    ...
    def can_data(self, msg):
        ...
        self.command_vel[1] = msg.linear.y
        self.command_vel[2] = msg.angular.z
    ...
    def control_callback(self, control_msg):
        ...
        self.control_n = control_msg.data
    ...
    def odom_callback(self):
        ...
        for k in range(self.Nmhe):
            ...
            self.U_est[k,0] = self.control_n[0]
            self.U_est[k,1] = self.control_n[1]
            self.U_est[k,2] = self.control_n[2]
            self.U_est[k,3] = self.control_n[3]
    ...
    def odom_pub_callback(self):
        ...
        print(np.round(self.sol_Xmhe[0],3))
        self.odom_publisher.publish(odom_msg)

    def control_pub_callback(self):
        control_msg = Float32MultiArray()
        u1 = float(np.round(self.sol_Umhe[0],3))
        u2 = float(np.round(self.sol_Umhe[1],3))
        u3 = float(np.round(self.sol_Umhe[2],3))
        u4 = float(np.round(self.sol_Umhe[3],3))
