...
class NMPCRabbit(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("controller_topic", None)
        ...
        self.controller_subscriber = self.create_subscription(String, controller_topic, self.controller_callback, 10)
        ...
        self.twist_cmd = self.create_subscription(Twist, 'cmd_vel', self.twist_callback, 10)
    ...
    def inverse_kiinematic(self, vx, vy, vyaw):

        J = (0.05/4)*np.array([
            [1, 1, 1, 1],
            [-1, 1, 1, -1],
            [-(0.20+0.020), (0.20+0.020), -(0.20+0.020), (0.20+0.020)]
        ], dtype=np.float64).T

        inv_vec = J@np.array([vx, vy, vyaw], dtype=np.float64)
    ...
    def twist_callback(self, cmd_msg):
        ...
        self.vx = cmd_msg.linear.x
        self.vy = cmd_msg.linear.y
        self.vyaw = cmd_msg.angular.z

    def nmpc_solver(self):

        if self.controller_state == "no_mpc":

            inv_vec = self.inverse_kiinematic(self.vx, self.vy, self.vyaw)

            self.opt_u1 = inv_vec[0]
            self.opt_u2 = inv_vec[1]
            self.opt_u3 = inv_vec[2]
            self.opt_u4 = inv_vec[3]
        elif self.controller_state == "mpc":
