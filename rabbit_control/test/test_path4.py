...
goal3 = np.vstack([x3, y3, np.linspace(2.8, 3.0, 50)])
...
x4 = path4[:, 0]
y4 = path4[:, 1]
yaw4 = np.append(np.arctan2(np.diff(y4), np.diff(x4)), -0.52)

goal4 = np.vstack([x4, y4, yaw4])

# goal = np.hstack([goal1, goal2, goal3])



# np.savetxt('/home/kenotic/ros2ocp_ws/src/rabbit_control/rabbit_control/path1.csv', goal, delimiter=",")
goal = np.loadtxt('/home/kenotic/ros2ocp_ws/src/rabbit_control/rabbit_control/path1.csv', delimiter=',', dtype=np.float32)
